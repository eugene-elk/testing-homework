...
## Про решение и запуск
Тесты покрывают 6 Bug_id:

Jest:
- bugId = 1
products.test.tsx - проверка работоспособности функции getAllProducts
- bugId = 7
cart.test.tsx - проверка работоспособности корзины
- bugId = 4
common.test.tsx - проверка скрытия гамбургера меню

Для удобства тестирования Jest сделан bash-скрипт, который запускает все тесты с различными багами, и сохраняет результаты в папку testResults

Hermiona:
- bugId = 8, 10
cart.hermione.js - проверка сообщения об успешном заказе и валидации номера
- bugId = 9
product.hermione.js - проверка отображения страницы товара (размер кнопки)

Для запуска Hermione:
- Поменять версию node на v16.20.1 с помощью nvm
Нужно убедиться, что во всех используемых терминалах используется эта версия (node -v)
- npm ci, node run build

В одном терминале: BUG_ID={id} npm start
В другом терминале: ./node_modules/.bin/hermione gui test/hermione/{имя}.hermione.js 
Запуск через скрипт может работать некорректно

По вопросам запуска и за пояснениями: t.me/losickij
